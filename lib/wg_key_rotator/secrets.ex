defmodule WgKeyRotator.Secrets do
  ...
  @default_registry "<server-local-ip>:5000"
  ...
  def render_env(repo_root, system_env \\ System.get_env()) do
    with {:ok, values} <- read_env_values(repo_root) do
      {:ok, env_contents(values, system_env)}
    end
  end
  ...
  defp env_contents(values, system_env) do
    registry = compose_env_value(system_env, "REGISTRY", @default_registry)
    image_tag = compose_env_value(system_env, "IMAGE_TAG", @default_image_tag)
  ...
  defp compose_env_value(system_env, "REGISTRY", default) do
    case system_env |> Map.get("REGISTRY") |> blank_to_nil() do
      nil -> registry_from_server_ip(system_env, default)
      value -> value
    end
  end

  defp compose_env_value(system_env, key, default) do
    system_env
    |> Map.get(key)
    |> blank_to_nil()
    |> case do
  ...
  defp registry_from_server_ip(system_env, default) do
